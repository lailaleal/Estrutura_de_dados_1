...
struct List {
    ...
    // Método para inserir um valor na penúltima posição da lista
    void insertPenultimate(int value) {
        // Se a lista tem tamanho <= 1, não faz nada
        if (size <= 1) return;

        Node* newNode = new Node(value); // Cria um novo nó com o valor fornecido
        Node* current = first;           // Começa no primeiro nó

        // Percorre até chegar no penúltimo nó
        while (current->next != last) {
            current = current->next;
        }

        // Insere o novo nó entre o penúltimo e o último nó
        newNode->next = last;            // Novo nó aponta para o último
        current->next = newNode;         // Penúltimo nó aponta para o novo nó
...
int main() {
    ...
    // Exemplo de uso do método insertPenultimate
    cout << "Digite um valor para inserir na penúltima posição: ";
    cin >> inputValue;
    l.insertPenultimate(inputValue);
    cout << "Lista após inserir o valor na penúltima posição: ";
    l.print();
