...
struct List {
   ...
   // }
    ...
    void removeInicio() {
        ...
        Node* toDel = inicio;  // Armazena o inicio nó para deletar
        inicio = inicio->next;  // Atualiza o inicio para o próximo nó
...
// 1. Criar um método que recebe um número N e remove os N últimos elementos da lista. Caso o tamanho da lista seja menor que N, esvaziar a lista.
// O usuário irá digitar o número de elementos que ele deseja remover no final da lista.
void Atv1 (int n) { // função para remover no final
    int limite = n;
    if (limite > size()){// Se o limite for maior que o tamanho da lista,
        limite = size(); // Considere o limite o tamanho da lista
    }
       for (int i=0; i<limite; i++){ // A função vai percorrer 0 = 1 vez, remover o ultimo termo, 1 = 2 removendo outro termo, 2 = 3 removendo outro termo até
        // n = n + 1 que será o ultimo termo removido.
        removeFim();
        c--;
    }
    // Forma objetiva para se fazer
    //for (i=0; i<n; i++){ // A função vai percorrer 0 = 1 vez, remover o ultimo termo, 1 = 2 removendo outro termo, 2 = 3 removendo outro termo até
        // n = n + 1 que será o ultimo termo removido.
        //removeFim();}
}
...
void Atv2 () {
    if (size () < 2) return;  // Se o tamanho da lista for menor que 2 ou <=1 o metodo não fará nada
    // removeByPos (2); pesquisar este método em algum código
     if (size () == 2 ) removeFim(); // se o tamanho de minha lista é igual a 2, irei querer remover o final
     // Se não
        Node* toDel = inicio -> next; //vou criar um aux toDel que ira receber a segunda posição
        inicio -> next = toDel -> next; // a segunda posição irá receber a terceira posição
        delete(toDel);
        c--; //como irá remover um valor decrementar um valor
}
...
void Atv5 (int value){
    if (size() < 2) return;
     // FORMA OBJETIVA E FACIL insert (value, size() - 1)  // penultima é a ultima posição -1.
     // FAZENDO NA MÃO
     // Encontrar a penultima posição
     Node* aux = inicio;
