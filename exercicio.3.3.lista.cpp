...
struct List {
    ...
    // Método para remover o segundo elemento da lista (sem efeito em listas com <= 1 elemento)
    void removeSecond() {
        if (size <= 1) return;            // Se a lista tiver 0 ou 1 nó, não faz nada
        Node* toDelete = first->next;     // Marca o segundo nó para exclusão
        first->next = toDelete->next;     // Faz o primeiro nó apontar para o terceiro
        delete toDelete;                  // Deleta o segundo nó
        size--;                           // Reduz o tamanho da lista
        if (size == 1) last = first;      // Se a lista agora tiver 1 elemento, atualiza o último nó
    }
    ...
    // Novo método que adiciona o tamanho atual da lista ao final
    void appendSize() {
        pushBack(size);  // Insere o valor do tamanho da lista no final dela
    }
...
int main() {
    ...
    // Imprime a lista antes de qualquer operação
    cout << "Lista inicial: ";
    l.print();

    // Testa o método appendSize
    l.appendSize();
    cout << "Lista após adicionar o tamanho no final: ";
    l.print();
